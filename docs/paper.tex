...
\IfFileExists{figures/dataset_pipeline.png}{%
  ...
  \fbox{
    ...
    $\rightarrow$ Labeling-Function Ensemble $\rightarrow$ Human Spot-Check Gate $\rightarrow$ Train \& Test Split \\
    \vspace{1ex}
    \end{minipage}
  }%
}
\caption{The weak-supervision data construction workflow: (a) Passage extraction and chunking from multi-page government circulars; (b) Labeling-function weak supervision, human spot-check validation, and partitioning into training and evaluation sets.}
\end{figure}
...
We use weak supervision to label the chunked passages \citep{ratner2017snorkel, mintz2009distant}. Rather than routing every chunk to a remote language model, each passage is scored by an ensemble of deterministic labeling functions, which makes the dataset reproducible offline and independent of API quota. The functions are: a weighted procurement lexicon (for example, ``tender'', ``bid security'', ``GeM'', ``procuring entity''), an administrative and human-resources lexicon that votes for the negative class (for example, ``holiday'', ``reservation'', ``seniority'', ``pay commission''), structural regular expressions matching amount thresholds and bid-process phrasing, and a document-level prior derived from the source portal. The weighted votes are combined into a margin; a passage is labeled relevant or non-relevant when the margin exceeds a threshold, and is otherwise abstained from rather than given a guessed label.

The data generation script incorporates two engineering mechanisms:
\begin{enumerate}
    \item \textbf{Deterministic Scoring}: identical inputs always yield identical labels, so the corpus and its annotations are exactly reproducible and require no external service.
...
To measure weak supervision label noise, human annotators spot-checked a stratified sample of the auto-labeled chunks and recorded the human-label agreement rate. Discrepancies were primarily confined to introductory preambles that cited general government acts without stating procurement policies. Verified corrections were written back to the training dataset.
